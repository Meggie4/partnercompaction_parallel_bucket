...
class DBImpl : public DB {
 public:
  ...
  ///////////////meggie
  virtual Status Write(const WriteOptions& options, bool multi, 
                        MultiWriteBatch* updates);
  ///////////////meggie
 ...
 private:
  ...
  //level0 bucket
  std::map<char, uint64_t> bucket_prefixes_;
  ...
  //multi mem, for parallel
  int memtable_size_;
  std::vector<MemTable*> mem_group_;
  std::vector<MemTable*> imm_group_;
  std::vector<port::AtomicPointer> has_imm_group_;
  bool mem_allocated_;
  int remaining_mems_;
  ThreadPool* compact_thpool_;
  ...
  void ParallelCompactMemTable(MemTable* mem, MultiCompactionState* compact);
  void CompactMultiMemTable();
  bool FindInMemTable(LookupKey& lkey, std::string* value, Status* s);
  MultiWriteBatch* BuildMultiBatchGroup(Writer** last_writer)
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  Status OpenPartnerTableWithLevel0(MultiCompactionState* mcompact, char prefix, FileMetaData* fm);
  ///////////////meggie
